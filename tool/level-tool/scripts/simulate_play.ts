 ...
 *     на поле не осталось ни одной пары — 4 пузыря, серией до первого хода
 *     (в счётчик `rescues` серия идёт одним событием). Обычный мердж досыпку НЕ вызывает:
...
export function simulate(level: HandoffLevelJson): PlayResult {
   ...
   * мета-превращение, 1 за склейку половинок). Предохранители стоят особняком:
   * при них не освободилось ничего, и число у каждого своё — тупик лечит ПОЛЕ и
   * получает полную пачку, серией до первого хода, а подсказка после промахов
   * лечит взгляд игрока и даёт один шар. Вместимость поля остаётся потолком:
   * штатные поводы в неё попадают сами, а страховке падать некуда, если поле
   * полное.
   *
  ...
  for (let guard = 0; guard < 5000; guard += 1) {
    ...
    if (!best) {
      /*
       * Страховка: пар нет, а очередь есть — досыпка приходит вне ритма, ПОЛНОЙ
       * пачкой в 4 шара. Она лечит поле, а не взгляд игрока, поэтому должна
       * срабатывать наверняка. Цикл сам возвращается сюда, пока хода не
       * появилось, то есть серия тянется до первого легального мерджа — как
       * перепроверка в прототипе. В счётчик серия идёт одним событием: «поле
       * встало один раз» и «поле вставало дважды» — разные факты.
       */
      if (queue.length) {
        if (!inRescueRun) { rescues += 1; inRescueRun = true; }
        spawn(4);
        continue;
      }
      return { won: false, categoriesDone: done, categoriesTotal: total, movesUsed: moves,
    ...
    if (a.cat.startsWith('half:')) {
      ...
      if (!hasLegalMove() && queue.length) {                             // повод 3
        if (!inRescueRun) { rescues += 1; inRescueRun = true; }
        spawn(4);
      }
    }
