 ...
 *     на поле не осталось ни одной пары — по 1 шару, серией до первого хода
...
export function simulate(level: HandoffLevelJson): PlayResult {
   ...
   * мета-превращение, 1 за склейку половинок). Страховка стоит особняком: там не
   * освободилось ничего, и она даёт по одному шару серией. Вместимость поля
   * остаётся потолком: штатные поводы в неё попадают сами, а страховке падать
   * некуда, если поле полное.
   *
  ...
  for (let guard = 0; guard < 5000; guard += 1) {
    ...
    if (!best) {
      /*
       * Страховка: пар нет, а очередь есть — досыпка приходит вне ритма, ОДНИМ
       * шаром за раз. Цикл сам возвращается сюда, пока хода не появилось, то
       * есть серия тянется до первого легального мерджа — как перепроверка в
       * прототипе. В счётчик серия идёт одним событием: «поле встало один раз»
       * и «поле вставало четыре раза» — разные факты.
       */
      if (queue.length) {
