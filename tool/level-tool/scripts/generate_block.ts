 ...
 * вердикты валидатора, а здесь модель живого игрока.
 *
 * С 04.08 это уже не только диагностика: доля дошедших — гейт приёмки
 * (`BLIND_WIN_MIN` в core/generateBlock.ts), поэтому в таблице все уровни и
 * должны показывать высокие проценты. Низкий процент здесь означает уровень,
 * принятый на СНЯТОМ гейте (последняя треть попыток), и это повод смотреть
 * руками. В оценку сложности D слепой прогон по-прежнему не входит: модель
 * знания игрока не откалибрована живыми наигровками.
 */
const blindRows = result.levels.filter((l) => l.blindPlay && !l.blindPlay.unavailable);
if (blindRows.length > 0) {
  console.log('\nСлепой прогон (гейт приёмки по доле дошедших, в оценку D не входит): '
