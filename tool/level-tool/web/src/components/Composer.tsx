...
const LEVEL_TOGGLES: {
  ...
  // Цепь-линию (`chain_line`) убрали 04.08: механика ведёт себя нестабильно, и
  // уровень с ней получался то проходимым, то нет от одного лишь сида. Тип
  // модификатора и сама механика в коде остались — вернуть галочку можно одной
  // строкой, когда цепь станет предсказуемой.
];
        ...
        <div className="grid c3">
          ...
          */}
          ...
          <DraftField
            ...
            tip="КОЛИЧЕСТВО мета-пар на каждой позиции блока, через запятую. Мета-пара — это одна связь «категория внутри категории»: собранная четвёрка не исчезает, а превращается в один пузырь со своим именем и уходит в другую категорию уровня (собрали WATER — слово WATER стало пузырём внутри KITCHEN). На поле такой пузырь ничем не отмечен, поэтому это самый дорогой для игрока рычаг и цена его измерена: на первых двухстах уровнях при трёх парах слепой игрок проигрывает 41% партий, при четырёх — 52%. Поэтому до уровня 200 потолок — две пары, включая спайк; дальше 3 на обычной позиции и 4 на спайке. Пусто — план строится сам ровными долями, раз в две декады один уровень идёт совсем без мета. Ноль в позиции — уровень из одних плоских категорий."
            hint="пусто — 1-2 на уровень, до L200 потолок 2"
...
const STAGE_SETTING: Record<string, string> = {
  ...
  'назначение слов': 'окно свежести слов и наполнение базы: категориям не хватает утверждённых слов, из которых два — опоры (слова, по которым игрок узнаёт категорию)',
  'слепой прогон': 'число категорий, «мета-пар на уровень» и редкость слов: модельный игрок, который читает слова, а не ответы, проигрывает этот уровень по лимиту ходов',
