...
function buildPool(
  ...
  for (let cat = 0; cat < index.categories.length; cat += 1) {
    ...
    /**
     * Пулу нужен ЗАПАС, а не ровно четыре слова.
     *
     * Назначение слов — задача точного покрытия: каждое из четырёх слов
     * категории обязано иметь ровно один дом среди категорий уровня. Категория
     * ровно с четырьмя играбельными словами проваливает её при первом же
     * пересечении: убрать нечего. Пока база была своя, это почти не мешало —
     * пулы шли по 6-8 слов. После импорта словаря оригинала медиана размера
     * пула стала 4 (каждая категория оригинала встречалась на одном уровне
     * со своей четвёркой), и уровень на 17 категорий перестал собираться
     * вообще: 48 попыток из 48 падали на покрытии.
     *
     * Запас зависит от размера уровня: чем больше категорий, тем вероятнее
     * пересечение. Категорий с пулом 5+ в базе 2074, с 6+ — 1787; это больше,
     * чем вся база до импорта, так что сужение пула не голодает.
     *
     * Тонкие категории при этом не выбрасываются: они ждут углубления пула
     * AI-проходом пайплайна (generate-category-candidates), и это штатный шаг.
     */
    const slack = c.categoryCount >= 10 ? 2 : 1;
    if (approved.length < 4 + slack) continue;
...
function selectCategories(
  ...
  const scored = rest
    .map((entry) => {
      ...
      /**
       * Глубина пула: категория ровно с четырьмя играбельными словами не даёт
       * сборщику ни одного варианта, и точное покрытие («у каждого слова ровно
       * один дом на уровне») ломается на первом же пересечении. После импорта
       * словаря оригинала таких тонких категорий стало 4364 из 6294, и вес 0.05
       * перестал их отталкивать: пресет 201-210 терял по уровню на двух seed
       * из двенадцати. 0.18 подобран замером: отказов ноль, приёмка декад цела.
       */
      const depthBonus = Math.min(entry.approved.length - 4, 6) * 0.18;
