 ...
 * gen-1.5 — старт раздаётся В ГЛУБИНУ (`autoScheme` в deal.ts): вход четвёркой,
 *           дальше тройки, остаток — одна пара, одиночек нет вовсе. Пачка
 *           досыпки закрывает до ДВУХ категорий, а не одну. Причина —
 *           наигровка 03.08: ровная раздача выкладывала 12 категорий по два
 *           слова, собрать после первой четвёрки было нечего, и прототип
 *           приходилось лечить раздутой досыпкой «добери поле до нормы».
 *           Хеши уровней меняются, и обязаны: уровень с другой выкладкой —
 *           другой уровень.
 * gen-1.6 — схема старта исполняется буквально (решение владельца 03.08:
 *           «4 это 4, а половинки в разных досыпках и мета не должны
 *           нарушать»). Доля схемы отдаётся категории, которая её реально
 *           держит (best-fit в `buildDeal`), а не очередной по списку с молчаливым
 *           урезанием до её размера: мета-родитель (спавнится три слова) больше
 *           не превращает четвёрку схемы в тройку. Распиленные слова уходят в
...
export function wordFitsGates(index: ContentIndex, word: number, gates: DecadeGates | null): boolean {
   ...
   * (словарь игры, см. `minWordWeight` в DecadeGates). Снимок без весов гейт
...
export function planDeepChain(
  ...
  for (let cat = 0; cat < index.categories.length; cat += 1) {
    ...
    // словаре игры с полом веса 0.70 категория с четырьмя approved-словами
...
function assignWords(
   ...
   *
   * Где у снимка есть ВЕС СВЯЗИ (словарь игры), берётся он, а не очевидность.
   ...
   * на той же шкале 0..1 (scripts/export_lexicon_snapshot.py). Смысл ровно тот
