...
export interface SnapshotCategory {
  ...
  /**
   * Только сводная база: откуда категория — индекс в `Snapshot.origins`
...
export interface SnapshotWord {
  ...
  tok: number;
  /**
   * Вес слова 0..1: годится ли оно в уровень вообще. Только сводная база
   ...
   * в scripts/export_hybrid_snapshot.py.
   */
  w?: number;
  /** 1, если вес оценён по уликам, а не прочитан с разметки регистра */
  we?: 0 | 1;
  /** только сводная база: откуда слово, индекс в `Snapshot.origins` */
  o?: 0 | 1 | 2;
}
...
export type SnapshotMembership = [
   ...
   * а замер: у сводной базы улика оригинала (доля выходов слова в четвёрках
   ...
   * сторону. Подробности — в scripts/export_hybrid_snapshot.py.
   */
  weight?: number,
  /** только сводная база: чья это связь, индекс в `Snapshot.origins` */
  origin?: 0 | 1 | 2,
];
...
export interface DecadeGates {
   ...
   *
   * Читается только там, где вес есть (сводная база, снимок 2.1). На нашей базе
