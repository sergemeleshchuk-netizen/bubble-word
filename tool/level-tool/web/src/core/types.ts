...
export interface BlockConfig {
  ...
  /**
   * Явный выбор категории-картинки по позициям блока: `1` — уровень обязан
   * получить мета-пузырь со значком, `0` — картинок на уровне нет, `null` —
   * на усмотрение генератора (доля от числа мета-пузырей, как было всегда).
   *
   * Появилось вместе с чекбоксом «🖼» на графике ритма 04.08. До него картинка
   * была чистой лотереей: доля 25% от мета-пузырей уровня плюс попадание имени
   * в словарь значков — на замере это давало картинку на четырёх уровнях блока
   * из десяти, и повлиять на это было нечем.
   *
   * Поле необязательное: пока его нет, нормализованный конфиг не меняется и
   * хеши уже собранных пакетов остаются прежними.
   */
  iconPlan?: (0 | 1 | null)[];
  /**
  ...
  /**
   * Сколько категорий обязано лежать на старте ЦЕЛИКОМ — готовыми четвёрками
   * (`DEAL_MIN_FULL_SETS` в core/deal.ts).
   *
   * Требование владельца продукта 04.08 после наигровки: «чтобы везде было
   * минимум 3 четвёрки — всё ещё неиграбельно для рядового не носителя языка».
   * Одна готовая четвёрка на старте (прежнее правило) даёт неносителю ровно один
   * очевидный ход, а дальше поле — тройки и пары незнакомых слов: гипотезу о
   * категории надо УГАДАТЬ, а не увидеть. Три готовые четвёрки означают три
   * хода, которые видно глазами, и три сбора, каждый из которых приносит
   * досыпку — то есть время разобраться в остальном поле.
   *
   * Цена — ширина старта: бюджет 24 пузыря при трёх четвёрках открывает 7 линий
   * вместо 8, остальные ждут в очереди. Это осознанный обмен ширины на
   * играбельность.
   *
   * Не задано = 1 (историческое поведение): так пакеты, собранные до 04.08,
   * сохраняют свои хеши.
   */
  ...
  /**
   * Сколько категорий уровень откладывает за гейт прогресса — «очередь линий»
   * (`planGates` в core/deal.ts). Работает только на уровнях крупнее
   * `QUEUE_STAGING_FROM` категорий: на поле из 24 пузырей больше двенадцати
   * живых линий одновременно означает, что собрать нельзя ничего.
   *
   * 0 или не задано = гейтов нет (так себя ведут пакеты, собранные до 04.08).
   */
...
export interface LevelPlan {
  ...
  poolRankTarget?: number | null;
  /**
   * Категория-картинка на уровне: мета-пузырь рисуется значком вместо слова
   * (`core/metaIcons.ts`).
   *
   *   `auto`    — как было до 04.08: доля от числа мета-пузырей, и что нашлось
   *               в словаре значков, то и нашлось. Умолчание.
   *   `require` — уровень ОБЯЗАН получить хотя бы одну; генератор при выборе
   *               мета-пар предпочитает имена, у которых значок есть.
   *   `forbid`  — картинок на уровне нет вовсе, даже если имена подходят.
   *
   * Требование законно отступает: если пул декады не даёт ни одного покрытого
   * имени, ослабление «картинка не обязательна» возвращает уровень в `auto` —
   * потерять значок лучше, чем потерять уровень.
   */
...
/**
 * Отложенная категория: её слова не выходят на поле, пока уровень не собран
 * до указанного прогресса. Правила и обоснование — в `core/deal.ts`.
 */
export interface DealGate {
  /** ключ категории (`LevelCategory.key`) */
  category: string;
  /** категорий уровня должно быть собрано, чтобы линия открылась */
...
export interface Deal {
  ...
  queue: DealBubble[];
  /**
   * Категории, отложенные до прогресса (см. `core/deal.ts`, `planGates`).
   * Клиент обязан их исполнять: пузырь отложенной категории не спавнится, пока
   * гейт закрыт, — досыпка берёт следующий подходящий из очереди. Поля нет =
   * гейтов нет, и выкладка ведёт себя ровно как до их появления.
   */
  gates?: DealGate[];
}
...
export interface LevelSpec {
  ...
  board: {
    ...
    dealMinStartWords?: number;
    /**
     * Сколько готовых четвёрок обязано лежать на старте. В спеке по той же
     * причине, что и режим раздачи: выкладка пересчитывается из одного спека,
     * и приёмка (`checkDeal`) должна знать, что от неё требовали. Нет поля = 1,
     * как у пакетов до 04.08.
     */
    dealMinFullSets?: number;
    ...
    dealStartBubbles?: [number, number];
    /**
     * Сколько категорий уровень держит за гейтом прогресса (см. `core/deal.ts`).
     * Лежит в спеке потому же, почему и остальные поля раздачи: выкладка обязана
     * пересчитываться из одного спека (`dealForSpec`). Нет поля или 0 = гейтов
     * нет, и выкладка та же, что была до них.
     */
    dealHoldCategories?: number;
  };
