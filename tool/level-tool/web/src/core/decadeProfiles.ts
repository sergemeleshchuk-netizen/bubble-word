...
function modifiersForDecade(from: number): Modifier[] {
  ...
  // Цепь-линия сюда не попадает (04.08): механика нестабильна, а галочки на
  // графике у неё больше нет — блок получал бы модификатор, которого не видно.
  return out;
}
...
export function configForRange(
  ...
  return {
    ...
    // Цепь-линия стояла с L51 на пик блока, но 04.08 её убрали: механика ведёт
    // себя нестабильно, поэтому лесенка её больше не выдаёт.
