 ...
 *     приходит 3 (одно место поле уже получило), склейка половинок освобождает
 *     одно место и приносит 1 пузырь. Числа фиксированные: сколько мест ушло,
 *     столько и пришло;
 ...
 *     цепь, потом тает лёд, потом досыпка «вне ритма» — полная пачка в 4 пузыря,
 *     серией до первого легального мерджа. А досыпка-подсказка после серии
 *     промахов — ОДИН пузырь: она лечит не поле, а взгляд игрока.
 *
...
export function createPlaySim(spec: LevelSpec): PlaySim {
  ...
  /**
   * Досыпка пачкой. `wave` — считать ли пачку волной РИТМА.
   *
   * Волна ритма — это пачка, от которой игрок ждёт развязки: 4 пузыря за
   * собранную категорию, 3 за мета-превращение, 4 на страховку. Один пузырь за
   * склейку половинок волной не считается: он не пачка, а возврат ровно одного
   * освободившегося места. Иначе уровень с восемью распилами набирал бы восемь
   * «непродуктивных волн» из ничего, и доля продуктивных волн — она уходит в
   * оценку сложности через dealShape — падала бы за наличие половинок, а не за
   * плохой ритм.
   */
  const spawn = (n: number, wave = true): void => {
  ...
  return {
    ...
    attempt(rawI: number, rawJ: number): AttemptResult {
       ...
       * стали одним, значит одно место освободилось и на него приходит ОДИН
       * пузырь. Без этого бот играл по правилам, которых у прототипа нет: на
       * уровне с восемью распилами он терял восемь пузырей поля и приходил к
       * вердикту по более бедному полю, чем видит живой игрок.
       */
      if (glued) spawn(1, false);
      ...
      /*
       * Страховка от тупика досыпает ПОЛНУЮ пачку — 4 шара (решение владельца
       * продукта 03.08). Она лечит ПОЛЕ: ходов физически нет, и вернуть его в
       * играбельное состояние надо наверняка. Бот зовёт rescue() в цикле, пока
       * легальной пары нет, то есть серия сама тянется до первого хода — ровно
       * как перепроверка в прототипе.
       *
       * В счётчик серия попадает ОДНИМ событием: «поле встало один раз, и его
       * разжали двумя пачками» и «поле вставало дважды» — разные факты, а гейт
       * приёмки читает именно этот счётчик.
       */
      if (queue.length > 0) {
    ...
    /*
     * Досыпка-подсказка после серии промахов — ОДИН шар (решение владельца
     * продукта 03.08). В отличие от страховки от тупика она лечит не поле, а
     * взгляд игрока: ходы на поле есть, он их не видит. Чинить нечего, и
     * четвёрка случайных слов работала против него — добавляла шума в поле,
     * которое и так запутало. Волной ритма один шар не считается, как и шар за
     * склейку половинок.
     */
    hintRefill(): boolean {
