 ...
 */
export const HANDOFF_KEY = 'bubble-level-tool.generated-pack.v2';
/**
 * Версии ключей растут не от смены формата, а от ОЧИСТКИ хранилища по просьбе
 * владельца: смена имени — единственный способ убрать старые пакеты у ВСЕХ, кто
 * уже открывал инструмент, до чужого браузера мы иначе не достаём. Старое имя
 * остаётся в хранилище мёртвым грузом на несколько килобайт и никем не читается.
 *
 * v2 архива (04.08, вечер) — в нём копились пакеты прежних правил (без правила
 * опоры, с четырьмя мета-парами, с прежним бюджетом ходов).
 * v3 архива и v2 рабочего слота (04.08, ночь) — правило «минимум три готовые
 * четвёрки на старте» переписывает выкладку каждого уровня, то есть все пакеты
 * в хранилище стали историей. На этот раз чистится и рабочий слот: там лежал
 * пакет, который прототип открывал сразу, и играть в старую выкладку, проверяя
 * новую, — ровно та путаница, из-за которой архив и заводили.
 */
export const HANDOFF_LIST_KEY = 'bubble-level-tool.generated-packs.v3';
