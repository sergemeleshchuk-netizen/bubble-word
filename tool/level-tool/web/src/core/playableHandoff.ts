...
/**
 * v2 (04.08, вечер) — не смена формата, а ОЧИСТКА архива по просьбе владельца:
 * в нём копились пакеты, собранные прежними правилами (без правила опоры, с
 * четырьмя мета-парами, с прежним бюджетом ходов). Рядом с новыми они читались
 * как «инструмент выдаёт то же самое», хотя это просто история.
 *
 * Смена имени ключа — единственный способ очистить хранилище у ВСЕХ, кто уже
 * открывал инструмент: до чужого браузера мы иначе не достаём. Старый ключ
 * остаётся в хранилище мёртвым грузом на несколько килобайт и никем не читается.
 */
export const HANDOFF_LIST_KEY = 'bubble-level-tool.generated-packs.v2';
