 ...
 *
 * Закреплено:
 *  1) авто-раздача кладёт на старт минимум три ГОТОВЫЕ четвёрки (требование
 *     владельца 04.08: «чтобы везде было минимум 3 четвёрки»), M=9 →
 *     4-4-4-3-3-3-3, и недобор четвёрок ловит приёмка выкладки;
 ...
 * троек и четвёрок много. 04.08 (вечер) требование усилено: «чтобы везде было
 * минимум 3 четвёрки — всё ещё неиграбельно для рядового не носителя языка».
 * Схема 4-4-4-3-3-3-3 — прямое следствие: три готовые четвёрки (три хода,
 * которые видно глазами, без угадывания категории), дальше тройки — им до сбора
 * не хватает одного слова. Было 4-3-3-3-3-3-3-2: одна готовая четвёрка и на
 * одну открытую линию больше, то есть ширина вместо играбельности.
 */
test('авто-превью M=9 — три готовые четвёрки, дальше тройки', () => {
  assert.deepEqual(liteSchemePreview(9), [4, 4, 4, 3, 3, 3, 3]);
});
...
// --------------------------------------------------------------------------- //
// минимум готовых четвёрок на старте
// --------------------------------------------------------------------------- //

/*
 * Требование владельца продукта 04.08 (вечер), после наигровки собранной
 * десятки: «добавь в конфиг, чтобы везде было минимум 3 четвёрки — всё ещё
 * неиграбельно для рядового не носителя языка».
 *
 * Проверяется то, что видит игрок: сколько категорий он может собрать СРАЗУ,
 * не угадывая и не дожидаясь досыпки. Мета-родитель здесь не считается: у него
 * на поле три слова из четырёх, а четвёртое родится сбором ребёнка.
 */
const easyBlock = generateBlock({
  snapshot, scoring, config: configForRange([121, 130], 'full-sets-test'),
});

/** Категорий, которые можно собрать со старта: четыре своих слова на поле. */
function collectableAtStart(spec: LevelSpec): number {
  const full = spec.board.wordsPerCategory;
  const counts = new Map<string, number>();
  for (const b of spec.deal.start) counts.set(b.category, (counts.get(b.category) ?? 0) + 1);
  return spec.categories.filter((c) =>
    c.words.filter((w) => w.kind !== 'meta').length >= full
    && (counts.get(c.key) ?? 0) >= full).length;
}

test('конфиг декады требует три готовые четвёрки, и уровни их дают', () => {
  assert.equal(configForRange([121, 130], 'x').dealMinFullSets, 3);
  assert.ok(easyBlock.levels.length >= 8,
    `декада собралась на ${easyBlock.levels.length}/10`);
  for (const level of easyBlock.levels) {
    assert.ok(collectableAtStart(level.spec) >= 3,
      `уровень ${level.spec.levelId}: собираемых со старта категорий `
      + `${collectableAtStart(level.spec)}, нужно 3`);
  }
});

/*
 * Требование обязано быть ПРИЁМКОЙ, а не пожеланием раздачи: иначе уровень, где
 * четвёрок не хватило (материала, места на поле), молча уехал бы в пакет.
 */
test('приёмка выкладки замечает недобор четвёрок', () => {
  const level = easyBlock.levels[0];
  // выкладка, посчитанная ПРЕЖНИМ правилом (одна четвёрка), против спека,
  // который требует три
  const old = dealForSpec({
    ...level.spec, board: { ...level.spec.board, dealMinFullSets: undefined },
  });
  const problems = checkDeal(level.spec, old);
  assert.ok(problems.some((p) => /готовых четвёрок/.test(p)),
    `приёмка недобор не заметила: ${problems.join('; ') || 'претензий нет'}`);
  assert.deepEqual(checkDeal(level.spec, level.spec.deal), [],
    'своя выкладка уровня претензий вызывать не должна');
});
