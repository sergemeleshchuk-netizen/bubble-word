 ...
 * важное: пресет блока 201-210 после всех изменений даёт тот же pack hash —
 * иначе сдаваемый пакет перестал бы воспроизводиться.
 *
...
/** pack hash сдаваемого пакета: зафиксирован в data/final-pack/pack.json. */
const FINAL_PACK_HASH = JSON.parse(
  readFileSync(join(ROOT, 'data/final-pack/pack.json'), 'utf8')).pack_hash as string;
...
test('пресет 201-210 после калибровки даёт тот же pack hash', () => {
