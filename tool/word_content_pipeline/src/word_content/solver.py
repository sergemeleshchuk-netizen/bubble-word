"""Локальные проверки на словах: пулы категорий и одна четвёрка.

Здесь живут две вещи, и обе — **не** доказательство корректности уровня:

  * `solve()` — разбиение набора слов на четвёрки без учёта значений слов.
    Быстрая проверка «на словах», которой пользуется сборка четвёрок.
  * `quartet_locally_unique()` — не лежит ли одна четвёрка целиком в чужом пуле.

Единственность полного уровня проверяет `level_solver.solve_level`: он работает
с токенами (слово + значение), знает структурные категории, умеет таймаут и
сохраняет отчёт о запуске. Разделение сделано намеренно: прежнее имя
`quartet_is_unique` читалось как «уровень с этой четвёркой однозначен», хотя
проверялась одна группа. Неоднозначность рождается на стыке групп, и локальная
проверка её не видит.
"""
...
def quartet_locally_unique(
    ...
    *,
    ...
    """Локальная проверка одной четвёрки: не лежит ли она целиком в чужом пуле.

    Слово «локальная» здесь не украшение. Проверка ничего не говорит о уровне:
    две локально однозначные четвёрки в одном уровне дают два ответа, если у них
    есть общее слово. Единственность уровня — `level_solver.solve_level`.
    """
    ...
    return SolverResult(
        ...
        reason="локально однозначна: другой категории с этими четырьмя словами нет",
    )
