...
DEFAULTS: dict[str, float] = {
    ...
    "label_quality_clarity_weight": 0.35,
    "label_quality_naturalness_floor": 0.35,
    "label_quality_specificity_weight": 0.15,
    "label_quality_display_weight": 0.15,
    "label_quality_familiarity_weight": 0.15,
...
# Слишком широкие названия: понятно, но принцип не сужен.
BROAD_WORDS = frozenset(
    {"animals", "food", "objects", "people", "places", "colors", "colours", "nature"}
)
...
def label_clarity(label: str) -> Explained:
    ...
    if len(words) == 1 and words[0] in BROAD_WORDS:
        parts["single_broad_word"] = -0.15
    return Explained(score=clamp(sum(parts.values())), parts=parts)


def label_specificity(label: str, pool_size: int) -> Explained:
    """Название не должно быть ни слишком широким, ни искусственно узким."""
...
def label_quality(
    *,
    ...
    parts = {
        ...
        "specificity": round(config["label_quality_specificity_weight"] * specificity, 4),
        ...
        conn.execute(
            ...
            INSERT INTO category_label_scores
                (category_id, label_char_count, label_token_count,
                 ...
                 label_novelty_score, label_quality_score, scoring_version, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                int(row["category_id"]),
        ...
        conn.execute(
            ...
            UPDATE quartets
               SET cohesion_score = ?, familiarity_score = ?,
                   ...
                   label_quality_score = ?, scoring_version = ?, updated_at = ?
             WHERE id = ?
            """,
            (
