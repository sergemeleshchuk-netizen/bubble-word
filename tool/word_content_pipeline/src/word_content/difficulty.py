...
Границы ответственности: это **pre-scoring эвристика отбора кандидатов внутри
базы**, а не оценка D готового уровня. Единой моделью D/F/R/C является
`tool/level-tool/web/src/core/` — см. `tool/docs/level_evaluation/evaluation_mapping.md` §1.
Число, посчитанное здесь, не имеет права расходиться с D уровня и при этом
считаться вторым мнением: оно нужно только для того, чтобы разложить кандидатов
по кривой до того, как уровень вообще собран.
"""
