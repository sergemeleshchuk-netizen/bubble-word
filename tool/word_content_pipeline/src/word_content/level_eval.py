...
TRAP_MIN = 0.03
# Потолок вклада ловушек в сложность. За ним уровень уже не «сложный»,
# а «нерешаемый», и это ловит порог отрыва разбиения, а не шкала.
TRAP_WEIGHT_MAX = 5.0
...
def _trap_weight(count: int) -> float:
    """Вклад ловушек в сложность: +1 за первые три, дальше +0.5, потолок +5.

    Потолок был +3 и оказался слепым пятном модели: уровень с десятью
    пересечениями получал ту же сложность, что уровень с тремя. Поймано на
    живом пакете — генератор ставил 5.2 пересечения на уровень против 1.2 в
    записи, модель показывала «лучше оригинала», а игрок на втором уровне
    сказал «очень сложно».

    Складываются ловушки не линейно: поле, где уже есть три спорных пузыря,
    воспринимается как трудное целиком, и четвёртая добавляет меньше третьей.
    Но добавлять не перестаёт.
    """
    if count <= 3:
        return float(count)
    return min(TRAP_WEIGHT_MAX, 3.0 + 0.5 * (count - 3))
