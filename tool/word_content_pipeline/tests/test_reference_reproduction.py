...
def test_18d_playable_export_keeps_the_meta_link_readable(db):
    ...
    Проверяем на пакете TOP, а не на RMK. Раньше здесь стоял RMK — ремейк
    двадцатки оригинала, — и у теста была скрытая цена: пакета RMK в базе почти
    никогда не оказывалось, тест молча уходил в skip, и контракт не проверялся
    вовсе. Пакет RMK удалён 04.08 (сдаём собственный блок 201-210), а TOP в базе
    есть и несёт 25 мета-связей.

    Почему не REF, где мета-связей больше всего (52). REF — это ЗАПИСЬ
    оригинала, а не наша сборка, и мета-пузырь там устроен иначе: группа LENGTH
    выпускает слово «ruler», то есть представителя, а не собственное имя. Наш
    контракт с прототипом на записи не выполняется по построению, и это факт про
    оригинал, а не дефект пайплайна: `applyLevel` узнаёт мета-пузырь совпадением
    слова с именем другой группы, поэтому мета-связь записанного уровня в
    прототипе мета-связью не выглядит.
    """
    pack = level_pack.build(db, "TOP")
    assert pack["levels"], "пакета TOP в базе нет — база собрана не до конца"
