...
Последний шаг возвращает ручные решения по 4967 связям — без него всё, что было
...
|---|---|---|---|
| `approved` | 12 859 | значение, которое игрок вспоминает первым; годится куда угодно | `monitor` -> COMPUTER PARTS |
| `alternative` | 42 | верное и узнаваемое, но не первое значение: **материал для ловушек** в обычном уровне | `monitor` -> HOSPITAL THINGS |
| `hard_only` | 4 588 | верно, но игрок сам не догадается: только сложные уровни | `monitor` -> LIZARDS (варан) |
...
Отдельно: у слов-омонимов **второстепенные значения не могут быть очевидными**.
Для каждого омонима перечислено, какие значения приходят игроку в голову первыми
(`approved`), какие он узнаёт вторым заходом (`alternative`, очевидность 0.5)
и до каких сам не дойдёт (`hard_only`, очевидность 0.25).
