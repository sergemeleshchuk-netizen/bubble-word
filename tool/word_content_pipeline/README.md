...
- Значения разведены у 223 слов-омонимов (549 значений). У остальных слов значение
...
## 14. Папка-выдача и пакет для ревью
...
├── база-слов.sqlite         снимок базы, 7.1 МБ, открывается DB Browser for SQLite
└── ревью/                   материалы для проверки человеком или внешней моделью
```

Снимок делается через `VACUUM`, то есть без мусора от импортов. Рабочая база
остаётся в `database/content.sqlite` и в git не хранится: источник правды —
текстовые файлы в `data/`, а не бинарник.
...
| `02_categories.md` | все категории: правило и слова со статусами (373 КБ) |
...
| `04_flags.md` | редкие слова, отклонённые, тонкие категории, категории с перевесом hard_only |
| `05_memberships.csv` | все связи в машинном виде (1.8 МБ) |
