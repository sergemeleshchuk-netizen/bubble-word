...
## 14. Пакет для внешнего ревью

```bash
python scripts/export_review_pack.py --db database/content.sqlite
```

Собирает `review/` — файлы, которые можно отдать человеку или загрузить в чат
внешней модели:
...
| `05_memberships.csv` | все связи в машинном виде (1.8 МБ, в git не коммитится) |
