...
Связь может быть верной и при этом неиграбельной. Генератор собрал группу
BOARD GAMES из `Life / risk / sorry / trouble` — все четыре настольные игры
существуют, и все четыре связи корректны. Играть невозможно: `life` читается как
жизнь, `risk` как риск, `sorry` как извинение, `trouble` как неприятность. Тему
приходится не узнавать, а вычислять исключением остальных групп.

База отвечала на все вопросы одним числом `fit_score`, у 17 505 связей из 18 882
равным 0.97. Вопросов на самом деле три, и они независимы:

| вопрос | где живёт ответ |
|---|---|
| может ли слово в этом значении принадлежать категории | `memberships.semantic_status` |
| вспомнит ли игрок это значение, увидев слово без надписи | `word_senses.accessibility_class` |
| хватает ли четвёрке ясных якорей, чтобы тему прочитать | `quartet_semantics` |

**Пять классов доступности.** `primary` — значение, которым слово читается без
контекста. `common_secondary` — не первое, но массово известное (`orange` как
цвет, `cold` как простуда). `specialist` — верное и нишевое (`Trouble` как
конкретная игра). `obscure` — редкое даже для поздней кампании. `unresolved` —
доступность не определена; такое значение в продакшен не идёт.

**Два числа рядом с классом.** `recognition_score` — узнает, если объяснить.
`activation_score` — вспомнит сам, увидев слово. Для названия игры первое
заметно выше второго, и одним числом это не записывается. Пустая оценка не
повышается до высокой: слот без чисел просто не становится якорем.

**Что отличает `orange` от `Trouble`.** Оба используют не главное значение
слова, то есть булев признак `uses_non_dominant` у них одинаковый. Различает их
`risk_class`: `fair_secondary` против `specialist_trick`. Именно его читает
генератор — иначе пришлось бы либо пропускать неиграбельные группы, либо
запрещать честные ловушки.

**Кто разбирается вручную, кто автоматически.** Слову с одним значением значение
выводится автоматически (`quality_source = derived_monosemous`), а оценки берутся
из уже измеренной знакомости, поэтому они разные у разных слов. Разбирается руками
слово, у которого значений больше одного, которое стоит в категории названий
(`categories.names_titles`), которое имя собственное или растащено по многим темам.
Решения лежат в `data/seed/_sense_map.json`; слово, вручную признанное
однозначным, живёт в `_not_homonyms.txt`.

```bash
word-content apply-sense-layer --db database/content.sqlite
word-content word-info --db database/content.sqlite --word trouble
```

**Признак категории, а не список слов.** `trouble` имеет ровно одну связь и по
разбросу тем не подозрителен вовсе — поймать его можно только через категорию:
BOARD GAMES держит названия, а не слова. Поэтому у категории есть флаг
`names_titles` (в seed — буква `T` или `P` в поле flags), и обычное английское
слово внутри такой категории автоматического значения не получает никогда.

**Профиль `first_lineup`** требует разрешённых значений у всех слотов, запрещает
`specialist` и `obscure`, разрешает одно `common_secondary` на группу и два на
уровень и требует три ясных якоря в лексической группе. Мета-коллекторы и правила
про написание из проверки якорей исключены: якорей у них нет по устройству.

**Коды отказа** (`quartet_semantics.REASON_CODES`): `UNRESOLVED_SENSE`,
`SPECIALIST_SENSE`, `OBSCURE_SENSE`, `TOO_MANY_COMMON_SECONDARY_SENSES`,
`INSUFFICIENT_CLEAR_ANCHORS`, `LOW_SENSE_ACCESSIBILITY`, `LOW_COHERENCE`,
`PROFILE_RULE_MISMATCH`, `META_DEPENDENCY_INVALID`,
`PACK_SWOW_DISCONNECTED_RATIO`.

### Связность четвёрок по SWOW

Датасет SWOW-EN лежит локально (144 МБ, лицензия research-only, в git не идёт).
Генератор его не читает: иначе чистая сборка на машине без датасета давала бы
другую базу. Вместо этого research-команда считает снимок, а сборка импортирует
готовый файл:

```bash
.venv/bin/python scripts/swow_quartet_metrics.py    # только там, где датасет есть
word-content import-swow-metrics --db database/content.sqlite
```

Снимок различает два нуля, и это принципиально: `observed_pairs = 0` означает,
что пару нечем было измерить (пробел в чужих данных), а `positive_pairs = 0` —
что измеряли и связи нет. Отклонять четвёрку можно только за второе.

SWOW не является универсальным порогом. Он применяется к ассоциативным и
функциональным правилам и не применяется к закрытым наборам, частям целого,
мета-коллекторам и игре слов: `north / south / east / west` держатся правилом
набора, а не попарными ассоциациями, и общий порог выбросил бы весь этот класс
контента.
