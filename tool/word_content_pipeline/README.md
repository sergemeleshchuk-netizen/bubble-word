...
|---|---|---|
| `approved` | 12 859 | годится для любого уровня |
| `hard_only` | 4 630 | связь верна, но неочевидна или слово редкое — только для сложных уровней |
...
`monitor` как экран — очевидно (0.85), `monitor` как варан — нет (0.30, `hard_only`),
хотя оба факта верны. Для каждого омонима перечислено, какие значения приходят
игроку в голову первыми; остальные получают потолок очевидности 0.3.
