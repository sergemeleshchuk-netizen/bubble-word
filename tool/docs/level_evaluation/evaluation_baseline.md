...
- **Принятых уровней ноль.** Пока `level_decisions` пуст, калибровать модель
  на «человеческой истине» нечем. Первые accepted-уровни появятся из текущей
  основной задачи (приёмка пяти кандидатов).
